...
impl Expander {
    ...
    pub fn expand(&mut self, shrub: &Shrubbery) -> Result<ExpandValue> {
        match shrub {
            ...
            Shrubbery::ShowRule { selector, transform, span } => {
                // Collect the show rule with kind derived from selector
                let sel = self.parse_selector(selector)?;
                self.rules.add_show_rule(ShowRule::new(sel, *transform.clone(), *span));
                Ok(ExpandValue::None)
            }
            ...
            Shrubbery::SetRule { selector, properties, span } => {
                ...
                // Validate properties against selector schema
                sel.validate_properties(&props, *span)?;
            ...
            Shrubbery::LiveBlock { deps, body, span: _ } => {
                // Live blocks are NOT expanded at expand-time
                // Instead, capture them as LiveCell for render-time execution by MrlKernel

                // Serialize the body to MRL source
                let source = self.serialize_body_to_source(body);
                ...
                let dep_names: Vec<String> = deps
                    ...
                    .map(|syms| {
                        syms.iter()
                            .filter_map(|sym| self.symbols.get(&sym.id()).cloned())
                            .collect()
                    })
                    .unwrap_or_default();

                // Create a LiveCell content node
                use crate::content::LiveCell;
                let live_cell = LiveCell {
                    source,
                    deps: dep_names,
                    cell_id: None, // Generated by renderer
                };

                Ok(ExpandValue::Content(Content::Live(live_cell)))
            }
    ...
    /// Serialize shrubbery body to MRL source text
    /// Used for capturing live blocks without expanding
    fn serialize_body_to_source(&self, body: &[Shrubbery]) -> String {
        let mut parts: Vec<String> = Vec::new();
        for item in body {
            parts.push(self.serialize_shrub_to_source(item));
        }
        parts.join("\n")
    }

    /// Serialize a single shrubbery node to MRL source
    fn serialize_shrub_to_source(&self, shrub: &Shrubbery) -> String {
        match shrub {
            Shrubbery::Literal(lit, _) => {
                use crate::shrubbery::Literal;
                match lit {
            ...
            Shrubbery::Identifier(sym, _, _) => {
                self.symbols.get(&sym.id())
                    .cloned()
                    .unwrap_or_else(|| format!("_id{}", sym.id()))
            }
            Shrubbery::Operator(sym, _) => {
                self.symbols.get(&sym.id())
                    .cloned()
                    .unwrap_or_else(|| format!("op{}", sym.id()))
            }
            Shrubbery::Parens(items, _) => {
                let inner: Vec<String> = items.iter()
                    .map(|i| self.serialize_shrub_to_source(i))
                    .collect();
                format!("({})", inner.join(" "))
            }
            ...
            Shrubbery::Prose(text, _) => text.clone(),
            ...
            Shrubbery::Quote { body, .. } => {
                format!("quote[{}]", self.serialize_shrub_to_source(body))
            }
            Shrubbery::Splice { expr, .. } => {
                format!("${}", self.serialize_shrub_to_source(expr))
            }
            Shrubbery::If { condition, then_branch, else_branch, .. } => {
                let cond = self.serialize_shrub_to_source(condition);
                let then_part = self.serialize_shrub_to_source(then_branch);
                match else_branch {
                    Some(else_br) => format!(
                        "if {}: {} else: {}",
                        cond, then_part, self.serialize_shrub_to_source(else_br)
                    ),
                    None => format!("if {}: {}", cond, then_part),
                }
            }
            Shrubbery::For { pattern, iterable, body, .. } => {
                let pat = self.symbols.get(&pattern.id())
                    .cloned()
                    .unwrap_or_else(|| format!("_p{}", pattern.id()));
                format!(
                    "for {} in {}: {}",
                    pat,
                    self.serialize_shrub_to_source(iterable),
                    self.serialize_shrub_to_source(body)
                )
            }
            _ => format!("{}", shrub), // Fallback to Display
        }
    }
    ...
    pub fn expand_with_rules(&mut self, shrub: &Shrubbery) -> Result<ExpandValue> {
        ...
        if let ExpandValue::Content(content) = value {
            ...
            if !self.rules.show_rules.is_empty() {
                ...
                content = apply_show_rules_with_kind(content, &rules, &mut |matched, transform, expected_kind| {
                    self.apply_show_transform(matched, transform, expected_kind)
                })?;
            }
    ...
    /// Enforces kind preservation: transform result must match expected_kind
    fn apply_show_transform(
        &mut self,
        matched: &Content,
        transform: &Shrubbery,
        ...
        // Enforce kind preservation
        let result_kind = if content.is_block() {
            ContentKind::Block
        } else if content.is_inline() {
            ContentKind::Inline
        } else {
            ContentKind::Content
        };
        ...
        let kind_ok = match (expected_kind, result_kind) {
            // Exact match
            (a, b) if a == b => true,
            // Content selector accepts any kind
            (ContentKind::Content, _) => true,
            // Block/Inline selectors require matching kind
            _ => false,
        };

        if !kind_ok {
            return Err(MrlError::KindMismatch {
                span: transform.span(),
                expected: format!("{:?}", expected_kind),
                got: format!("{:?}", result_kind),
            });
        }
